...
func handleAddExpense(w http.ResponseWriter, r *http.Request) {
	...
	if addRequest.PaymentMethod == "" {
		sendErrorResponse(w, "Payment method is required", http.StatusBadRequest)
		return
	}
	...
	// Set the ID on the expense object
	expense.ID = expenseID

	// Update user's balance based on payment method
	err = updateBalance(expense.UserID, -expense.Amount, expense.PaymentMethod)
	if err != nil {
		log.Printf("Error updating balance: %v", err)
		// Continue since the expense was already added
	}

	// Fetch the newly created expense from database
	newExpense, err := fetchExpenseByID(expenseID, expense.UserID)
	if err != nil {
		log.Printf("Error fetching new expense: %v", err)
		// Return the created expense without timestamps
		sendSuccessResponse(w, "Expense added successfully", expense)
		return
	}

	// Return the newly created expense with timestamps
	sendSuccessResponse(w, "Expense added successfully", newExpense)
}
