...
# =============================================================================
# SCRIPT PARA SOLUCIONAR EL PROBLEMA DE CATEGORIES UPDATE
# =============================================================================
# Este script obtiene un ID de categoría válido y prueba el endpoint update
# =============================================================================
...
echo -e "${WHITE}"
echo "============================================================================="
echo "   🔧 CATEGORIES UPDATE ENDPOINT - SOLUTION IMPLEMENTATION"
echo "============================================================================="
echo -e "${NC}"
...
get_existing_categories() {
    ...
    if [ "$status_code" -eq 200 ]; then
        echo -e "${GREEN}✅ Categorías obtenidas exitosamente${NC}"
        ...
        if [ -n "$category_id" ] && [ "$category_id" != "null" ]; then
            echo -e "${GREEN}📝 Primera categoría encontrada con ID: $category_id${NC}"
            echo "$category_id"
            return 0
        else
            echo -e "${YELLOW}⚠️ No se encontraron categorías existentes${NC}"
            return 1
        fi
    else
        echo -e "${RED}❌ Error obteniendo categorías: Status $status_code${NC}"
        echo "$content"
        return 1
    fi
}

# Función para crear una nueva categoría y obtener su ID
create_test_category() {
    ...
    if [ "$status_code" -eq 200 ]; then
        ...
        if [ -n "$category_id" ] && [ "$category_id" != "null" ]; then
            echo -e "${GREEN}📝 Nueva categoría creada con ID: $category_id${NC}"
            echo "$category_id"
            return 0
        else
            echo -e "${RED}❌ No se pudo extraer el ID de la categoría creada${NC}"
            return 1
        fi
    else
        echo -e "${RED}❌ Error creando categoría: Status $status_code${NC}"
        echo "$content"
        return 1
    fi
}
