...
main() {
    ...
    # Intentar conectar vía SSH (sin clave)
    log_info "Probando conexión SSH inicial..."
    if ssh -o ConnectTimeout=10 -o PasswordAuthentication=yes $VPS_USER@$VPS_HOST "echo 'Conexión SSH básica OK'" 2>/dev/null; then
        log_success "Conexión SSH básica funciona"
    else
        log_warning "Conexión SSH requiere configuración adicional"
    fi
